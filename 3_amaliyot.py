print('"Nexia"' ','  '"Tico"' ',' "'Damas'" , "ko`rganlar qilar havas")

#1) 5 ning 4-darajasini toping
print('5 sonining' , '4 - darajasi' , 5**4 ,  'ga teng') 

#2) 22 ni 4 ga bo`lganda qancha qoldiq qoladi
print("22 ni 4 ga bo`lganda", 22%4 , "qoldiq qoladi")

#3) Tomonlari 125 ga teng kvadratning yuzi va perimetrini toping
print("Tomonlari 125 ga teng kvadratning yuzi" , 125*125 , " ga teng" ',' "\nUning perimetri" , 4*125 , "ga teng")

#3a) 
print('Tomonlari 125 ga teng kvadratning yuzi' ,   125**2 ,  'ga teng') 
print('Uning perimet' ,   4*125 ,  'ga teng')

#3b)
print("Tomonlari 125 ga teng kvadratning yuzi" , 125*125 , " ga teng," , "Uning perimetri" , 4*125 , "ga teng")

#4) Diametri 12 ga teng bo`lgan doiraning yuzini toping (п-3,14 deb oling)
print("Diametri 12 ga teng bo'lgan doiraning yuzi S =" , 3.14*(12/2)**2, 'ga teng')

#5) Katetlari 6 va 7 bo`lgan to`g`ri burchakli uchburchakning gipotenuzasini toping.
print("Katetlari 6 va 7 bo`lgan to`g`ri burchakli uchburchakning gipotenuzasi" , (6**2+7**2)**(1/2), "ga teng")#
